...
static void sigchld_handler(int sig) {
  ...
  while ((pid = waitpid(-1, &status, WUNTRACED | WCONTINUED | WNOHANG)) > 0) {
    ...
    job_t *job = NULL;
    proc_t *proc = NULL;
    // Znajdz jaki to job i jaki proces
    for (int i = 0; i < njobmax; i++) {
      number_of_proceses = jobs[i].nproc;
      for (int j = 0; j < number_of_proceses; j++) {
    ...
    // Jesli wszytskie procesy maja zmieniony status zmien status joba
    int i;

    for (i = 0; i < number_of_proceses && job->proc[i].state == proc->state;
         i++)
      ;
    // debug("|%d %d|",number_of_proceses , i);
    if (i == number_of_proceses) {
...
#ifdef STUDENT
  msg("continue '%s'\n", jobcmd(j));
  // wyslij sygnał do wznowienia grupie procesów
  Kill(-1 * jobs[j].pgid, SIGCONT);
  // poczekaj az działa
  while (jobs[j].state != RUNNING) {
    Sigsuspend(mask);
  }
  // jesli nie bg to daj na fg i montoruj
  if (!bg) {
    movejob(j, 0);
    monitorjob(mask);
  }
...
#ifdef STUDENT
  // zabij i wznów by mogły umrzec
  Kill(-1 * jobs[j].pgid, SIGTERM);
...
#ifdef STUDENT
    // jesli nie wołana pomiń
    if (jobs[j].state != which && which != -1) {
      continue;
    }
    // jesli i ma odpowiadajacy status wypisz odpowiedni komunikat
...
#ifdef STUDENT
  // przekazanie kontroli nad terminalem programowi
  Tcgetattr(tty_fd, &shell_tmodes);
  Tcsetpgrp(tty_fd, jobs[0].pgid);
  // poczekaj az sie zakończy
  while ((state = jobstate(0, &exitcode)) == RUNNING) {
    Sigsuspend(mask);
  }
  // jeśli zastopowana to wrzuc do tła
  if (state == STOPPED) {
    int job = allocjob();
    movejob(0, job);
  }
...
void shutdownjobs(void) {
  ...
  for (int i = BG; i < njobmax; i++) {
    // jesli zakonczona to pomin jelsi nie o zakoncz i poczekaj na zakonczenie
