...
noreturn void external_command(char **argv) {
  ...
  if (!index(argv[0], '/') && path) {
    ...
    char *program = strtok(newpath, srednik);
    /* Używając strtok wybieram kolejne sciezki i wykonuje dla nich execve */
    while (program != NULL) {
      char copy[1000];
      strcpy(copy, program);
      strcat(copy, "/");
      strcat(copy, argv[0]);
      execve(copy, argv, environ);
      program = strtok(NULL, srednik);
    }
    free(program);
    free(newpath);
#endif /* !STUDENT */
